...
class PointClusteringHelper(points: FloatBuffer) {
    ...
    private fun allocateGrid(points: FloatBuffer) {
        ...
        val numCellsX = Math.max(
            1,
            Math.ceil(((bounds.maxX - bounds.minX) / GRID_CELL_SIZE).toDouble()).toInt()
        )
        val numCellsY = Math.max(
            1,
            Math.ceil(((bounds.maxY - bounds.minY) / GRID_CELL_SIZE).toDouble()).toInt()
        )
        val numCellsZ = Math.max(
            1,
            Math.ceil(((bounds.maxZ - bounds.minZ) / GRID_CELL_SIZE).toDouble()).toInt()
        )
        occupancyGrid = Array(numCellsX) {
            Array(numCellsY) {
                BooleanArray(
                    numCellsZ
                )
        ...
        while (points.hasRemaining()) {
            ...
            val indexX = Math.floor(((x - gridOriginOffset[0]) / GRID_CELL_SIZE).toDouble()).toInt()
            val indexY = Math.floor(((y - gridOriginOffset[1]) / GRID_CELL_SIZE).toDouble()).toInt()
            val indexZ = Math.floor(((z - gridOriginOffset[2]) / GRID_CELL_SIZE).toDouble()).toInt()
